...
import { HubState } from "../packages/utils/HubState";
...
import { UnitConversionPreset } from "../packages/utils/units";
import { createUUID, jsonCopy } from "../packages/utils/util";
import ExportOptions from "../packages/utils/ExportOptions";
import TabType, { getAllTabTypes, getDefaultTabTitle, getTabIcon } from "../packages/utils/TabType";
import {
  DEFAULT_PREFS,
...
import { createExtraFRCDataFolder, loadFRCData } from "../packages/FRC/frcDataUtil";
import StateTracker from "./StateTracker";
import videoExtensions from "../packages/utils/videoExtensions";
...
// I Apologize for the following code I didn't write the base of it and I'm not fixing this nested mess
  ...
  switch (message.name) {
    ...
    case "live-rlog-start":
      ...
      rlogSockets[windowId].setTimeout(RLOG_CONNECT_TIMEOUT_MS, () => {
        sendMessage(window, "live-rlog-data", {
  ...
  const template: (Electron.MenuItemConstructorOptions | Electron.MenuItem)[] = [
    ...
    },
    {
      ...
      submenu: [
          ...
          click(_, window) {
            ...
            dialog
              .showSaveDialog(window, {
                ...
                defaultPath: "FRC-In-Control " + new Date().toLocaleDateString().replaceAll("/", "-") + ".json",
                properties: ["createDirectory", "showOverwriteConfirmation", "dontAddToRecent"],
                ...
                if (files.filePaths.length > 0) {
                  ...
                  if (data.version !== currentVersion) {
                    let result = dialog.showMessageBoxSync(window, {
                      ...
                      detail:
                        "The layout file was generated by a different version of FRC In-Control. Compatability is not guaranteed.",
        ...
        {
          label: "Import NT4 Config...",
          click(_, window) {
            if (window == null || !hubWindows.includes(window)) return;
            dialog
              .showOpenDialog(window, {
                title: "Select a config file to import",
                properties: ["openFile"],
                filters: [{ name: "JSON files", extensions: ["json"] }]
              })
              .then((files) => {
                if (files.filePaths.length > 0) {
                  ...
                  if (data.version !== currentVersion) {
                    let result = dialog.showMessageBoxSync(window, {
                      ...
                      message: "Version mismatch",
                      detail:
                        "The config file was generated by a different version of FRC In-Control. Compatability is not guaranteed.",
                      buttons: ["Continue", "Cancel"],
                      icon: WINDOW_ICON
                    });
                    if (result != 0) return;
...
function createHubWindow() {
  ...
  window.loadFile(path.join(__dirname, "../www/pages/hub.html"));
  return window;
}
...
function createEditRangeWindow(
  ...
  editWindow.loadFile(path.join(__dirname, "../www/pages/editRange.html"));
}
...
function createUnitConversionWindow(
  ...
  unitConversionWindow.loadFile(path.join(__dirname, "../www/pages/unitConversion.html"));
}
...
function createRenameTabWindow(
  ...
  renameTabWindow.loadFile(path.join(__dirname, "../www/pages/renameTab.html"));
}
...
function createExportWindow(parentWindow: Electron.BrowserWindow, currentLogPath: string | null) {
      ...
      if (event.data === null) {
        ...
        dialog
          ...
          .then((response) => {
            if (!response.canceled) {
              exportWindow.destroy();
              sendMessage(parentWindow, "prepare-export", {
  ...
  exportWindow.loadFile(path.join(__dirname, "../www/pages/export.html"));
}
...
function createSatellite(parentWindow: Electron.BrowserWindow, uuid: string, type: TabType) {
  ...
  satellite.loadFile(path.join(__dirname, "../www/pages/satellite.html"));
...
function openPreferences(parentWindow: Electron.BrowserWindow) {
  ...
  const width = 400;
  const height = process.platform == "win32" ? 276 : 260; // "useContentSize" is broken on Windows when not resizable
  ...
  prefsWindow.loadFile(path.join(__dirname, "../www/pages/preferences.html"));
}
...
function openDownload(parentWindow: Electron.BrowserWindow) {
  ...
  downloadWindow.loadFile(path.join(__dirname, "../www/pages/download.html"));
}
